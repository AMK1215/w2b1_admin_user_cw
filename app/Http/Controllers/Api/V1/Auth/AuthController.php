...
class AuthController extends Controller
{
    ...
    private const PLAYER_ROLE = 2; // Player role ID in 3-role system (Owner=1, Player=2, SystemWallet=3)
    ...
    public function register(RegisterRequest $request)
    {
        ...
        if (! $owner) {
            return $this->error('', 'Owner not found', 401);
        }
        ...
        $user = User::create([
            ...
            'agent_id' => $owner->id, // agent_id = owner_id
    ...
    public function getAgent()
    {
        $player = Auth::user();
        // Note: agent_id is actually owner_id (Owner->Player relationship only)
        // This returns the player's owner information

        return $this->success(new AgentResource($player->parent), 'Owner Information');
    }
